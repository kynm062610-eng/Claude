---
name: sns-writing
description: Use when writing SNS captions/posts (X, Instagram, TikTok) or blog/article copy. Triggers on "キャプション", "投稿文", "SNS 文章", "ブログ記事", "キャッチコピー" などのリクエスト。動画の台本を書く場合は video-script スキルを優先。
---

# SNS投稿・ブログ ライティングスキル（2026年9月時点の情報ベース）

Web検索で集めた最新のキャプション/コピーの書き方をまとめたもの。半年〜1年経ったら
`WebSearch`で再確認してから使うこと。

## 大きな流れの変化

- 「完璧に見せる文章」より「人間らしい・素の文章」が評価される流れ。
- ハッシュタグを大量に積む時代は終わり、キャプション内に自然にキーワードを入れる方が
  検索経由の到達に効く（SNS内検索対策としてキーワード > ハッシュタグ）。

## プラットフォーム別

**Instagram / TikTok**
- 短く・パンチの効いたフックで始める。絵文字・質問・強めのCTAを使う。
- 目安は50〜150文字くらいの短さが好まれる。ミニストーリーやトレンド乗っかりが効く。
- **ハッシュタグはInstagramなら3〜5個**に厳選、ニッチで関連性の高いものに絞る（2026年9月更新：
  以前の5〜10個からさらに絞る方向。AIの文脈理解が高度化し、タグを大量につけるとスパム判定の
  原因になる）。代わりにキャプション本文へ検索キーワードを自然に含めるSEO対策が主流。
  TikTokはニッチ+広めのタグを混ぜる。
- **キャプションには「送信（シェア）」を促す一言を入れる**（2026年9月確認）。Instagramの評価軸は
  「保存数」から「送信数（誰かにDM/ストーリーズでシェアされたか）」にシフトしてる。
  「これパートナーに送って」「知り合いにも教えたい」と思わせるフレーズが効く。

**X (旧Twitter)（2026年8月確認）**
- 本当にトレンドに乗ってる時だけ1〜2個ハッシュタグをつける程度。基本は文章勝負。
- **リプライの往復が最重要指標**（いいねの約75倍のスコア）。投稿末尾に「AとB、どちらで迷いますか？」
  のようなオープンクエスチョンを入れて、リプライを誘発する。
- **滞在時間も高評価**（いいねの約22倍）。長文投稿・スレッドで「読ませる」構成が有利。
- **外部リンクは本文に貼らない**（最大40%減点）。YouTube/TikTokなどへ誘導したい時は、
  自己リプライ（ツリーの2つ目）にリンクを貼るか「詳細はプロフィールから」と案内する。
- **投稿後2時間の初速が勝負**。ターゲット層がスマホを見る時間帯（朝7〜9時、夜20〜22時）を
  狙って投稿し、自分からも他ユーザーにリプライしに行く。
- **アカウントのテーマを一貫させる**（「今日はラーメン、明日は投資」のようなブレはNG）。
  bio（160文字）で「誰に・何のメリットを与えるか」を明確に宣言する。
- **X Premium加入は伸ばす上で有利**（フォロワー内最大4倍、非フォロワー最大2倍のインプレッション
  優遇、長文投稿機能も使える）が、月額課金が発生するので加入は本人の判断で決める。
- 収益化の公式プログラム（フォロワー500人＋3ヶ月で500万インプレッション条件）を狙って過激な
  投稿（インプレゾンビ行為）に走ると、凍結・表示制限のリスクが高い。王道は「Xを集客窓口にして
  noteなどの有料コンテンツへ誘導する」動線（`income-strategy`参照）。

**Threads（2026年9月確認）**
- 評価される指標は「いいね」より**返信（リプライ）の連鎖**。投稿の最後に必ず
  「みんなはどう思う？」的なオープンクエスチョン（CTA）を入れて、リプライのハードルを下げる。
- 「AIが書いたような無機質な正論・まとめ」は伸びにくい。**本人の生々しい体験談・失敗談・
  リアルな数字**（一次情報）が評価される。
- 投稿直後の1時間の初速が重要。ついたリプライには即座に返信してスレッドを盛り上げる。
- フォロワー0の初期は、伸びてる同ジャンル投稿への**質の高い引用・リプライ**でプロフィールへの
  流入を稼ぐのが定石。

## プロフィール文（bio）の文字数上限（2026年9月時点）

プロフィール文を作る時は、書く前に必ずこの上限を確認してから文章を組み立てること
（文字数オーバーで書き直しにならないように）。ただし上限は変わることがあるので、
半年以上経ってたら`WebSearch`で最新の数字を取り直すこと。

| プラットフォーム | bio文字数上限 |
|---|---|
| X（旧Twitter） | 160文字（表示名は50文字まで） |
| Instagram | 150文字 |
| Threads | 150文字 |
| YouTube | ハンドル横の短い自己紹介は160文字。別途「概要」欄はもっと長く書ける（数百〜1000文字程度） |

## 書き方のコツ（共通）

- 冒頭の一文＝フック。ここで「答えが気になる」情報ギャップを作ると続きを読ませられる。
- 完璧な文章より、人が話してるみたいな温度感を優先する。
- 結論・フックを先に、説明は後。前置きを長くしない（動画台本と同じ考え方）。

## 出典（2026年9月時点）
- [Social Media Caption Strategies for Engagement 2026](https://bemysocial.com/social-media-caption-strategies/)
- [How to Write Social Media Captions That Actually Get Engagement (2026 Guide)](https://www.krumzi.com/blog/how-to-write-social-media-captions-that-actually-get-engagement-2026-guide)
- [Viral Instagram & TikTok Caption Trends 2026](https://captionory.com/blog/viral-instagram-and-tiktok-caption-trends-2026-whats-working-right-now)
- [Caption Character Limits 2026 — Instagram, X, TikTok, LinkedIn, Facebook](https://wildandfreetools.com/blog/caption-character-limits-2026-instagram-x-tiktok-linkedin/)
- [Social Media Character Limits 2026: The Complete Cheat Sheet](https://www.limitora.com/blog/social-media-character-limits-2026.html)
- [Threads Bio Character Limit 2026](https://momentumhive.app/blog/threads-bio-character-limit-2026)
- NotebookLMリサーチ経由（2026年9月、ユーザー提供）：Instagram/Threads 2026年アルゴリズム
  攻略まとめ（送信数シフト、Raw Content優遇、Threadsの返信連鎖評価など）
- NotebookLMリサーチ経由（2026年9月、ユーザー提供）：X（旧Twitter）2026年8月最新アルゴリズム
  まとめ（リプライ往復75倍評価、外部リンク40%減点、Premiumブーストなど）
