...
Kling生成は時間もコストもかかる。**毎回作らない。**

### 共通素材ライブラリ
```
assets/video/
├── loops/     … M1（語りの基本）を4シーン × 表情3種で作り置き
├── endings/   … M6 + M7 の締めクリップ（全動画で使い回す）
└── bg/        … ユキミなしの背景ループ（雪・雨・炎）
```

- **締め（④）は全動画で同じクリップを使い回す** → ブランド記号になり、制作も楽
- 語りベース（M1）は数本作れば、字幕を変えるだけで何本も作れる
- 新規生成が必要なのは、**特別な回だけ**
