 ...
 * 「確認した」を押すまで消えない。
 */
function describe(event: GuardianEvent): string {
  if (event.event_type === 'watch_mode_changed') {
    const to = String(event.detail.to ?? '');
    if (to === 'full') return 'おうちの人が、ノートを 見られる せっていに かえたよ。';
    if (to === 'notify_only') return 'おうちの人には「書いたこと」だけ つたわる せっていに かわったよ。';
    return 'おうちの人が、ノートを 見ない せっていに かえたよ。';
  }
  return 'おうちの人が、きろくの かくにんを もうしこんだよ。';
}
...
export function GuardianEventBanner({ childId }: { childId: string }) {
  ...
  return (
    <View style={styles.banner}>
      <Text style={styles.title}>おしらせ</Text>
      <Text style={styles.text}>{describe(event)}</Text>
      <BigButton label="わかった" variant="secondary" onPress={() => void acknowledge(event.id)} />
    </View>
  );
}
